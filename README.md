...
"🔑 구글 계정으로 로그인" 버튼을 누르면 새 팝업 창에서 구글 로그인이 진행됩니다.
팝업에서 로그인을 마치면 팝업이 자동으로 닫히고, 원래 탭이 자동으로 새로고침되며 앱으로 진입합니다.
...
- 이메일을 잘못 입력하고 저장하면 본인도 접근이 막힐 수 있습니다. 이 경우 서버에서
  ...
  게이트가 다시 꺼집니다.
- **보안 관련 트레이드오프**: Streamlit은 탭(세션)마다 로그인 상태가 독립적이라,
  팝업에서 로그인한 결과를 원래 탭이 곧바로 알 방법이 없습니다. 이를 위해 로그인 성공 시
  **서버 전체에 24시간짜리 임시 통행증**을 발급하는 방식을 씁니다 — 즉, 누군가 한 번
  로그인하면 그 24시간 동안은 URL을 아는 다른 방문자도 로그인 없이 접근할 수 있습니다.
  허용 이메일 1개만 실제로 로그인을 통과할 수 있으므로 통행증 발급 자체는 안전하지만,
  발급된 통행증은 세션이 아닌 서버 단위로 공유된다는 점을 유의하세요.
  더 엄격한 탭별 격리가 필요하면 이 방식 대신 매번 같은 탭에서 로그인 링크를 열고 돌아오는
  기존 방식으로 되돌릴 수 있습니다.
- "🚪 로그아웃"을 누르면 이 임시 통행증도 즉시 무효화됩니다.
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-07-26 | 구글 로그인 팝업 방식 전환, 네이버 로그인을 앱 화면에서 캡차 포함 직접 처리 가능하도록 추가 |
