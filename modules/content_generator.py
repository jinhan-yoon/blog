...
def suggest_topics(keywords: list[str], count: int = 10) -> list[dict]:
    kw_str = ", ".join(keywords)
    research_block = _format_research_block(research_keywords(keywords))
    prompt = f"""당신은 IT 전문 블로그를 운영하는 SEO 전문가이자 콘텐츠 전략가입니다.
이 블로그는 앞으로 IT 분야(소프트웨어, AI, 앱·서비스, 디바이스, 개발, 보안,
클라우드 등)에 집중합니다.

참고용 오늘의 트렌드 키워드 목록입니다 (IT와 무관한 항목은 무시하고, IT
관점에서 파생되는 각도를 찾거나 목록에 없어도 최근 관심 받는 IT 주제를
직접 골라도 됩니다):
{kw_str}
{research_block}

IT 분야에서 최근 관심 받는 주제 중, 검색량은 어느 정도 있지만 이를 다룬
문서(블로그 글)는 아직 적은 롱테일 키워드를 {count}개 정도 추천해주세요.

이 블로그는 신생/소규모 블로그라 대형 IT 매체·경쟁 블로그가 이미 상위를
차지한 키워드로는 검색 노출이 사실상 불가능합니다. 아래 기준을 반드시
지켜주세요:
- 키워드 자체(제품명, 서비스명 등 한 단어짜리 대표 키워드)를 그대로 주제로
  삼지 마세요. 경쟁이 너무 치열해 순위 노출 가능성이 낮습니다.
- 대신 그 키워드에서 파생되는 더 구체적인 롱테일 하위 주제·각도를 고르세요
  (사용법, 비교, 오류 해결, 설치·설정 가이드, 대안 추천, 실사용 후기 관점 등).
  검색량이 극단적으로 높은 대표 키워드보다, 검색될 가능성은 있으면서 이미
  다룬 문서 수는 적은 지점을 우선하세요.
- 반대로 너무 좁아서 아무도 검색하지 않을 주제도 피하세요. "검색될 가능성"과
  "문서 경쟁 강도" 둘 다 균형 있게 고려해서 고르세요.

추천한 {count}개를 초보 블로거가 상위 노출을 노리기 가장 좋은 순서(경쟁이
가장 낮고 노출 가능성이 높은 것부터)로 정렬해서, 그 순서 그대로 목록에
담아주세요.

반드시 아래 JSON 형식으로만 답하세요 (마크다운 없이 순수 JSON 배열, 정렬된
순서 그대로):
[
  {{
    "topic": "주제명",
    ...
    "reason": "이 주제를 선택한 이유 — 검색 가능성, 문서 수(경쟁 강도), 상위노출 난이도를 함께 언급 (1-2문장)"
  }}
]"""
