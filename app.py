...
# ════════════════════════════════════════════════════════
    ...
    if not st.session_state.post_content_html:
        ...
        _provider_info = {
            "pollinations": "🆓 Pollinations.ai — API 키 불필요, AI 신규 생성 (느릴 수 있음)",
            "huggingface":  "🤗 HuggingFace SD XL — HF 무료 토큰 필요, AI 신규 생성",
            "claude":       "🤖 Claude + Pollinations — Anthropic 키 필요, 프롬프트 강화 후 AI 신규 생성",
            "dalle":        "💰 DALL-E 3 — OpenAI 유료 키 필요, AI 신규 생성 (최고 품질)",
        }
...
# ════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        _opts     = ["pollinations", "huggingface", "claude", "dalle"]
...
vLLM 불가 시 자동으로 Claude로 전환. <code>claude-sonnet-4-6</code> 기본값.
            ...
            st.markdown("### 🖼️ 이미지 생성 (4가지 프로바이더, 전부 AI 신규 생성)")
            st.markdown("""
<div class="tech-card">
<b>Pollinations.ai</b> <span class="tech-badge">무료 · 기본값</span><br>
API 키 없이 URL 파라미터로 이미지를 생성하는 무료 서비스. 실패 시 자동 fallback 대상이기도 함.<br><br>
...
최고 품질 AI 이미지. OpenAI API 키 필요.<br><br>
<small>※ 저작권자가 있는 실사 스톡 사진(Picsum 등)은 사용하지 않고, 전부 AI로 새로 생성된 이미지만 사용합니다.</small>
</div>
            """, unsafe_allow_html=True)
...
**해결**:
- 미디어 탭에서 이미지 생성 방식을 **HuggingFace SD XL** 등 다른 AI 프로바이더로 변경 후 재시도
- 잠시 후 다시 Pollinations 시도 (일시적 지연인 경우가 많음)
            """)
